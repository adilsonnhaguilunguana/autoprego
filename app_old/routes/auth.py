...
def forgot_password():
    ...
    if not user:
        flash('Se este email existir no sistema, enviaremos instruções de recuperação.', 'info')
        return redirect(url_for('auth.autenticacao', tab='forgot'))

    # Aqui você poderia implementar envio de email com token
    # Exemplo simples:
    flash('Um link de recuperação foi enviado para seu email.', 'success')
    return redirect(url_for('auth.autenticacao'))
